...
const RANGE = 2;
export default function Pagination({ stylePagination, setQueryConfig, queryConfig, pageSize }) {
    ...
    const renderPageNumber = (pageNumber) => {
      return (
        <span
          ...
          onClick={() => setQueryConfig({ ...queryConfig, page: pageNumber.toString() })}
    ...
    <div className='pagination' style={{ ...stylePagination }}>
      {currentPage > 1 && (
        <span
          ...
          onClick={() => setQueryConfig({ ...queryConfig, page: (currentPage - 1).toString() })}
